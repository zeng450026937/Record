    ...
    _is_resume(false)
{
    ...
    for(int i = 0; i < 4; i++){
        ...
        more_menu->setStyleSheet("QMenu{ font: 9pt;border:0;margin:0;padding:0;}"
    ...
    connect(_notify_dialog,SIGNAL(notifyAccepted()),this,SLOT(accept_notify()));
    connect(_notify_dialog,SIGNAL(notifyRejected()),this,SLOT(reject_notify()));

    connect(more_menu,SIGNAL(triggered(QAction*)),this,SLOT(more_menu_triggered(QAction*)));
    connect(_scene_record_modellist,SIGNAL(modelList_selected(QVariant)),this,SLOT(get_modellist_selected(QVariant)));

    connect(ui->title,SIGNAL(textChanged(QString)),this,SLOT(conference_info_changed(QString)));
    connect(ui->context,SIGNAL(textChanged(QString)),this,SLOT(conference_info_changed(QString)));
    connect(ui->member,SIGNAL(textChanged(QString)),this,SLOT(conference_info_changed(QString)));
    connect(_sharedData->GetModel(ModelUpdater::TemplateModel),SIGNAL(rowsRemoved(QModelIndex,int,int)),this,SLOT(get_template_update(QModelIndex,int,int)));
    connect(_sharedData->GetModel(ModelUpdater::TemplateModel),SIGNAL(rowsInserted(QModelIndex,int,int)),this,SLOT(get_template_update(QModelIndex,int,int)));
    ...
    connect(_sharedData,SIGNAL(download_notify(int,QString,int,bool)),this,SLOT(receive_download_notify(int,QString,int,bool)));
    connect(_sharedData,SIGNAL(record_notify(QString,QString,QString)),this,SLOT(receive_record_notify(QString,QString,QString)));
}
...
int Scene_Record::aboutToStop()
{
    ...
    if(!_conf_uuid.isEmpty()){
        ...
        QPoint pos = this->parentWidget()->parentWidget()->parentWidget()->geometry().topLeft() + ui->ControlStackedWidget->geometry().center();
        _scene_record_finish->move(pos.x() - _scene_record_finish->width() / 2, pos.y() - _scene_record_finish->height() / 2);
        ...
        if ( _scene_record_finish->exec() == QDialog::Accepted ) {
            ...
            _sharedData->StopConf(_conf_uuid);
        }
        else{
            result = -1;
        }
...
void Scene_Record::init_model()
{
    ...
    sortFilter_proxyModel->setSourceModel( _sharedData->GetModel(ModelUpdater::DeviceModel) );
    ...
    ui->MarkTable->verticalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
...
void Scene_Record::on_add_mark(QString text)
{
    ...
    if(!_conf_uuid.isEmpty()){
        ...
        _sharedData->AddMark(RecorderShared::RecordMarkModel ,_conf_uuid, mark_info);

        ui->MarkTable->resizeColumnToContents(0);
        ui->MarkTable->horizontalHeader()->setSectionResizeMode(1,QHeaderView::Stretch);
...
void Scene_Record::get_template_update(QModelIndex parent,int first,int last)
{
    Q_UNUSED(parent);
    Q_UNUSED(first);
    Q_UNUSED(last);
...
void Scene_Record::on_rec_start_btn_clicked()
{
    if(_conf_uuid.isEmpty()){
        ...
        _sharedData->StartConf(_conf_uuid);
    }
}
...
void Scene_Record::more_menu_triggered(QAction *action)
{
    if (action->text() == "更多"){
        ...
        QPoint pos = this->parentWidget()->parentWidget()->parentWidget()->geometry().topLeft() + ui->InfoArea->geometry().center();
        _scene_record_modellist->move(pos.x() - _scene_record_modellist->width() / 2, pos.y() - _scene_record_modellist->height() / 2);
...
void Scene_Record::on_stop_rec_btn_clicked()
{
    this->aboutToStop();
}
...
void Scene_Record::receive_device_select_stateChanged(bool checked, QVariantMap device)
{
    QString mac = device.value("device_mac").toString();
    if(checked)
        _conf_device << mac;
    else
...
void Scene_Record::receive_download_notify(int type, QString uuid, int percentage, bool completed)
{
    Q_UNUSED(type);
    Q_UNUSED(completed);
    if(uuid == _conf_uuid){
...
void Scene_Record::receive_conference_notify(int state, bool result, QVariantMap conf)
{
    switch(state){
    case RecorderShared::kConfCreated:
        if(result){
            _conf_info = conf;
            _conf_uuid = conf.value("uuid").toString();

            ui->rec_file_name_lable->setText(_sharedData->GetFolder(1,_conf_uuid));
...
void Scene_Record::receive_record_notify(QString device_mac, QString device_name, QString status)
{
    if(!_conf_device.contains(device_mac))
        return;

    QString message;

    if(status == "SpeechWithNoConference" && !!!reminder_timer && _conf_uuid.isEmpty()){
    ...
    else if(status == "SpeechVoiceTooLow" && !!!reminder_timer && !_conf_uuid.isEmpty()){
        message = QString("\"%1\"\345\275\225\351\237\263\350\256\276\345\244\207\345\243\260\351\237\263\350\276\203\345\260\217\357\274\214\350\257\267\346\243\200\346\237\245\351\272\246\345\205\213\351\243\216\346\210\226\345\260\206\350\256\276\345\244\207\351\235\240\350\277\221\345\217\221\350\250\200\344\272\272").arg(device_name);
        _notify_dialog->NotifyMessage(message);
    }
    else if(status == "SpeechWithNoise" && !!!reminder_timer && !_conf_uuid.isEmpty()){
        message = QString("\"%1\"\345\275\225\351\237\263\350\256\276\345\244\207\344\271\213\345\210\253\345\210\260\346\235\202\351\237\263\357\274\214\350\257\267\350\260\203\346\225\264\344\270\213\350\256\276\345\244\207\350\277\234\347\246\273\345\231\252\345\243\260\346\272\220").arg(device_name);
        _notify_dialog->NotifyMessage(message);
    }
    else if(status == "SpeechNormal"){
        if(_notify_dialog->isVisible())
            _notify_dialog->hide();
    }
}
...
void Scene_Record::on_infoButton_clicked(bool checked)
{
    Q_UNUSED(checked);
...
void Scene_Record::on_pause_btn_clicked(bool checked)
{
    if(!_conf_uuid.isEmpty()){
        if(checked){
            _sharedData->PauseConf(_conf_uuid);
            ui->pause_btn->setChecked(false);
        }
        else{
            _sharedData->StartConf(_conf_uuid);
