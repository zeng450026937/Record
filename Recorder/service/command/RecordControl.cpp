...
#define RC_ENVIRONMENT_EXCEPTION "environmentException"

RecordControl::RecordControl(RecorderShared *pRecorderShare,
                             MessageBase *pMessager)
    : CommandBase(pMessager), m_pRecordShared(pRecorderShare) {
  qRegisterMetaType<QVector<int>>();
  AddActionProc(MB_INFO_MODE, RC_GET_DEVICE_LIST,
                &RecordControl::GetDeviceListReply);

  AddActionProc(MB_INFO_MODE, RC_UPDATE_DEVICE_INFO,
                &RecordControl::UpdateDeviceInfo);
  AddActionProc(MB_INFO_MODE, RC_ENVIRONMENT_EXCEPTION,
                &RecordControl::EnvironmentException);

  AddActionProc(MB_CONFERENCE_MODE, RC_CREATE_CONFERENCE,
                &RecordControl::CreateConferenceReply);
  AddActionProc(MB_CONFERENCE_MODE, RC_START_CONFERENCE,
                &RecordControl::StartConferenceReply);
  AddActionProc(MB_CONFERENCE_MODE, RC_PAUSE_CONFERENCE,
                &RecordControl::PauseConferenceReply);
  AddActionProc(MB_CONFERENCE_MODE, RC_STOP_CONFERENCE,
                &RecordControl::StopConferenceReply);
}
...
void RecordControl::GetDeviceListReply(bool bResult,
...
void RecordControl::EnvironmentException(bool bResult,
                                         const QJsonObject &jsData) {
  // if (bResult)
  {
    QVariantMap lsEnvironmentInfo = jsData.toVariantMap();
    if (lsEnvironmentInfo.value("needNotify").toBool())
      m_pRecordShared->receive_deviceEnvironmentUpdate(lsEnvironmentInfo);
  }
}
...
void RecordControl::CreateConferenceReply(bool bResult,
                                          const QJsonObject &jsData) {
...
void RecordControl::StartConferenceReply(bool bResult,
                                         const QJsonObject &jsData) {
...
void RecordControl::PauseConferenceReply(bool bResult,
                                         const QJsonObject &jsData) {
...
void RecordControl::StopConferenceReply(bool bResult,
                                        const QJsonObject &jsData) {
