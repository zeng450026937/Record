...
int ClipFileDatabaseImpl::AddFile(QString uuid, QString identity, int start_time, QString path)
{
    QMutexLocker locker(_shared->_apiLock);

    QSqlQuery query(_shared->SqlDatabase());
    query.prepare("insert or replace into tb_clip_file (uuid, identity, start_time, path) "
                "values (?, ?, ?, ?)");
...
QVariantList ClipFileDatabaseImpl::GetFileList(QString uuid)
{
    ...
    query.prepare("select * from tb_clip_file where uuid = ? order by start_time");
    query.addBindValue( uuid );
...
QVariantList ClipFileDatabaseImpl::GetConferenceFile(QString uuid)
{
    ...
    query.prepare("select * from tb_clip_file where uuid = ? AND start_time < '0' order by start_time");
    query.addBindValue( uuid );
