...
bool RecordDownloadReceiver::CreateReciveData(
    ...
    if (pDownloadDB->GetDownloadInfo(qstrFileUuid, qstrConferenceUuid,
        ...
        *iStartPos = m->pData->iWritten;
        ...
        m->pData->qstrFilePath = QString("%1\\%2%3")
        ...
        if (iType == RecorderShared::RT_CONFERENCE)
        {
            ...
            if (!downloadDir.exists())
                downloadDir.mkdir(m->pData->qstrFilePath);

            m->pData->qstrFilePath += QString("\\%1.%2").arg(qstrUserName,qstrFileExtension);
        }
        else
        {
            m->pData->qstrFilePath += QString("_%1.%2").arg(qstrUserName, qstrFileExtension);
        }
