...
{
    QDateTime dtCreateTime = QDateTime::fromString(info["createTime"].toString(),"yyyy-MM-dd HH:mm:ss");
    ...
    if (_info.isEmpty()) {
        ...
        _download_status = RecordDownloadReceiver::GetDownloadStatus(info["fileUuid"].toString(), 
            info["conferenceUuid"].toString(), info["deviceUuid"].toString());
...
void ListForm::CheckAndAliveData()
{
    ...
    RecordDownloadReceiver *pReceiver = pService->GetDownloadReciver(_info["fileUuid"].toString(), 
