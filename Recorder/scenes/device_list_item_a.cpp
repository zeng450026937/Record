...
DeviceListItemA::~DeviceListItemA()
{
    delete ui;
}
...
void DeviceListItemA::update_display(QVariantMap info)
{
    if(_info != info){
        ...
        int iper = _info.value("battery").toString().toInt();

        if(iper > 60){
            ui->batteryLabe->setStyleSheet("border-image: url(:/resource/u1405.png);");
        }else if(iper > 20){
            ui->batteryLabe->setStyleSheet("border-image: url(:/resource/u1412.png);");
        }else{
            ui->batteryLabe->setStyleSheet("border-image: url(:/resource/u1418.png);");
        }

        ui->batteryLabe->setText(_info.value("battery").toString()+"%");
