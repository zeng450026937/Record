...
Scene_File::Scene_File(RecorderShared *sharedData, QWidget *parent)
  ...
  connect(ui->time_slider, SIGNAL(set_value_changed(int)), this,
          SLOT(time_slider_set_changed(int)));

  connect(_player, SIGNAL(durationChanged(qint64)), this,
          SLOT(durationChanged(qint64)));
  connect(_player, SIGNAL(positionChanged(qint64)), this,
          SLOT(positionChanged(qint64)));
  connect(_player, SIGNAL(mediaStatusChanged(QtAV::MediaStatus)), this,
          SLOT(mediaStatusChanged(QtAV::MediaStatus)));
  connect(_player, SIGNAL(stateChanged(QtAV::AVPlayer::State)), this,
          SLOT(statusChanged(QtAV::AVPlayer::State)));
  connect(_player, SIGNAL(startPositionChanged(qint64)), this,
          SLOT(startPositionChanged(qint64)));
  connect(_player, SIGNAL(stopPositionChanged(qint64)), this,
          SLOT(stopPositionChanged(qint64)));
...
void Scene_File::init_filter_menu() {
  ...
  filter_menu->setStyleSheet(
      "QMenu{ font: 9pt;border:0;margin:0;padding:4px;}"
      "QMenu::item {"
...
void Scene_File::init_model() {
  ...
  ui->file_mark_tableView->verticalHeader()->setSectionResizeMode(
  ...
  ui->file_mark_tableView->setModel(
      _sharedData->GetModel(ModelUpdater::MarkModel));
  ui->file_mark_tableView->setWordWrap(true);

  ClipButtonDelegate *_clip_button =
      new ClipButtonDelegate(QObject::tr("剪辑"), this);
...
void Scene_File::update_mark_model(QVariantList &list) {
  ...
  if (list.count() > 0) {
    ...
    ui->file_mark_tableView->horizontalHeader()->setSectionResizeMode(
...
QString Scene_File::dateToReg(QDate startDate, QDate endDate) {
  ...
  if ((endyear - startyear) > 0) {
    ...
    RegText += differentMonthDayReg(startyear, startmonthStr, startday, endyear,
                                    endmonthStr, endday);
  } else {
    if ((endmonth - startmonth) > 1) {
      ...
      RegText += differentMonthDayReg(startyear, startmonthStr, startday,
                                      endyear, endmonthStr, endday);
...
QString Scene_File::differentMonthDayReg(int startyear, QString startmonthStr,
                                         int startday, int endyear,
                                         QString endmonthStr, int endday) {
  QString RegText;
...
void Scene_File::menu_triggered(QAction *action) {
  ...
  for (int i = 0; i < count; i++) {
    if (filter_menu->actions().at(i) == action) continue;
...
//     QPoint pos =
//     this->parentWidget()->parentWidget()->parentWidget()->geometry().topLeft()
//     + ui->file_listView->geometry().center();
//     _scene_file_dl->move(pos.x() - _scene_file_dl->width() / 2, pos.y() -
//     _scene_file_dl->height() / 2);
//
...
void Scene_File::on_confirm_btn_clicked() {
  ...
  QPoint pos = this->parentWidget()
                   ...
                   ->geometry()
                   .topLeft() +
               ui->rightWidget->geometry().center();
  _scene_file_clip->move(pos.x() - _scene_file_clip->width() / 2,
                         pos.y() - _scene_file_clip->height() / 2);

  QFileInfo fileinfo(_file.value("path").toString());

  QString output_name =
      fileinfo.dir().dirName() + " - " + _mark.value("mark").toString();
...
void Scene_File::time_slider_selected_changed(int value) {
  QTime time;
  time.setHMS((value / 3600) % 60, (value / 60) % 60, value % 60,
              (value * 1000) % 1000);
  ...
  time.setHMS((seconds / 3600) % 60, (seconds / 60) % 60, seconds % 60,
              (seconds * 1000) % 1000);
  ...
  _player->setTimeRange(
      (ui->time_slider->get_start_value() - _offset) * _time_unit,
      (ui->time_slider->get_end_value() - _offset) * _time_unit);

  if (!_player->isPlaying()) {
    _player->play();
  }
  _player->setPosition((ui->time_slider->get_start_value() - _offset) *
                       _time_unit);
  _player->pause(false);
}
...
void Scene_File::durationChanged(qint64 duration) {
  if (duration) {
    ...
    _total_time.setHMS((_duration / 3600) % 60, (_duration / 60) % 60,
    ...
    for (int i = 0; i < _mark_list.count(); i++) {
      ...
      if (time > _duration || time < _offset) _mark_list.removeAt(i);
    }
    // update mark
    this->update_mark_model(_mark_list);
...
void Scene_File::positionChanged(qint64 progress) {
  ...
  _current_time.setHMS((_progress / 3600) % 60, (_progress / 60) % 60,
                       _progress % 60, (_progress * 1000) % 1000);
...
void Scene_File::mediaStatusChanged(QtAV::MediaStatus status) {
  ...
  switch (status) {
    ...
    case QtAV::MediaStatus::LoadedMedia:     // if auto load and source is set.
                                             // player is stopped state
    ...
    case QtAV::MediaStatus::LoadingMedia:  // when source is set
      ...
      break;
    case QtAV::MediaStatus::StalledMedia:  // insufficient buffering or other
                                           // interruptions (timeout, user
                                           // interrupt)
      setStatusInfo(tr("Media Stalled"));
      break;
    case QtAV::MediaStatus::EndOfMedia:  // Playback has reached the end of the
                                         // current media. The player is in the
                                         // StoppedState.
      ui->play_btn->setChecked(false);
      _player->setPosition(0);
      _player->pause(true);
      break;
    case QtAV::MediaStatus::InvalidMedia:  // what if loop > 0 or stopPosition()
                                           // is not mediaStopPosition()?
...
void Scene_File::statusChanged(QtAV::AVPlayer::State status) {
  switch (status) {
    case QtAV::AVPlayer::State::StoppedState:
      ...
      break;
    case QtAV::AVPlayer::State::PlayingState:  /// Start to play if it was
                                               /// stopped, or resume if it was
                                               /// paused
...
void Scene_File::startPositionChanged(qint64 position) { Q_UNUSED(position); }
void Scene_File::stopPositionChanged(qint64 position) { Q_UNUSED(position); }
...
void Scene_File::setStatusInfo(const QString &info) {
  ...
  if (!_statusInfo.isEmpty())
      ui->mark_lineEdit->setText(QString("%1 |
  %2").arg(_trackInfo).arg(_statusInfo));
  else
...
void Scene_File::on_next_mark_btn_clicked() {
  if (ui->file_mark_tableView->model()->rowCount() <= 0) return;
  ...
  if (!ui->file_mark_tableView->currentIndex().isValid()) {
    ...
    current = current >= (_mark_list.count() - 1) ? (_mark_list.count() - 1)
                                                  : (current + 1);
  }

  ui->file_mark_tableView->selectRow(current);

  this->on_file_mark_tableView_clicked(
      ui->file_mark_tableView->model()->index(current, 0));
}
...
void Scene_File::on_file_mark_tableView_clicked(const QModelIndex &index) {
  ...
  if (_player->stopPosition() <= _duration * _time_unit) return;
  ...
  if (!ui->file_mark_tableView->hasFocus()) ui->file_mark_tableView->setFocus();
}
