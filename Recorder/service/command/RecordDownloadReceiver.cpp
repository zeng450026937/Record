...
bool RecordDownloadReceiver::StartReceiveTrigger(int iResult, int iFileSize)
{
    ...
    else
    {
        ...
        case RecordDownloadReceiver::EC_ARGS_INVALID:
            emit download_prompt(QString::fromUtf16((const ushort*)L"下载失败，下载请求参数无效。"));
            break;

        case RecordDownloadReceiver::EC_FILE_UNEXSISTS:
            emit download_prompt(QString::fromUtf16((const ushort*)L"下载失败，下载文件不存在。"));
            break;

        default:
            emit download_prompt(QString::fromUtf16((const ushort*)L"下载失败，未知的异常错误."));
            break;
        }
    }
